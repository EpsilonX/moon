...
public class Cache {
	...
	public Note getNote(int noteid) throws CacheException {
		...
		try {
			...
			if (Integer.parseInt(objr.getString("result")) < 0) {
				...
				note = new Note(obj.getInt("id"),
						StringEscapeUtils.unescapeHtml(obj.getString("title")),
						obj.getInt("root"), StringEscapeUtils.unescapeHtml(obj
								.getString("nodes")),
						obj.getString("createdate"), obj.getInt("nodenum"));
	...
	public int addNote(String title) throws CacheException {
		...
		try {
			...
			if (Integer.parseInt(objr.getString("result")) < 0) {
				...
				Note note = new Note(obj.getInt("id"), obj.getString("title"),
						obj.getInt("root"), obj.getString("nodes"),
						obj.getString("createdate"), obj.getInt("nodenum"));
				dbHelper.insertNote(note);
				return note.getId();
			}
	...
	public int updateNote(Note note) throws CacheException {
		...
		try {
			...
			if (Integer.parseInt(obj.getString("result")) < 0) {
				...
				Log.d("Cache.updateNote()", "update a Note to server! id = "
						+ note.getId());
				Note newnote = new Note(note.getId(), note.getName(),
						note.getRoot(), note.getJson(), note.getDate(),
						note.getNodenum());
		...
		try {
			...
			if (obj.has("result")) {
				if (Integer.parseInt(obj.getString("result")) < 0) {
					...
					Log.d("Cache.delNote()", "delete note id=" + noteid
							+ " success!");
	...
	public Node addNode(Node node, int noteid) throws CacheException {
		...
		pairs.add(new BasicNameValuePair("parentid", Integer.toString(node
				.getFather().getId())));
	...
	/**
	 * change node to the target position,if target = -1 then changed it to its
	 * father's layer
	 * 
	 * @param userid
	 * @param target
	 ...
	 * @throws CacheException
	 */
	public Node changePosition(int target, Node node, int noteid)
			throws CacheException {
		...
		List<NameValuePair> pairs = new ArrayList<NameValuePair>();
		pairs.add(new BasicNameValuePair("parentid", Integer.toString(father
				.getId())));
		pairs.add(new BasicNameValuePair("brotherid", Integer
				.toString(brotherid)));
	...
	public Bitmap getImage(String url, int option) {
		...
		if (option == 0) {// get img from db
			...
			// 得到新的bitmap
			Bitmap bit = Bitmap.createBitmap(bitmap, 0, 0, width, height,
					matrix, true);

			dbHelper.insertIMG(url, Transform.getInstance().bmp2String(bit));
			Log.d("Cache", "store in db");
